; Use the Phase Lock Loop to control the speed of the system bus (i.e. clock speed)

    ; Allow other files to call these labels    
    EXPORT pllInit      ; initialize the PLL            in:  r0=sysdiv2_divider
                        ;                               out: none
                        ;                               mod: r0-r3
    EXPORT pllSetDiv    ; set speed of the PLL          in:  r0=sysdiv2_divider
                        ;                               out: none
                        ;                               mod: r0-r3                
...
pllInit
        ...
        POP {r0}
        ORR r2, r2, r0, LSL #22         ; R2 = R2 + (SYSDIV2<<22) (configure SYSDIV2 and SYSDIV2LSB fields in RCC2)
        STR R2, [R3]                    ; [R3] = R2
...
pll_settle
        ...
        BEQ pll_settle                  ; if(R0 == 0), keep polling
...
pllSetDiv
        ...
        ; Clear current sysdiv2
        LDR r1, =(~SYSCTL_RCC2_SYSDIV2_M)
        AND r2, r2, r1
